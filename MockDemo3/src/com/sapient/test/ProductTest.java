...
public class ProductTest {
	...
	public static void beforeAll(){
		...
		p3 = new Product(1003,"samsung galaxy7",12000);
		p4 = new Product(1004,"samsung s8",24000);
		...
		p2spec = Arrays.asList("8gb ram","4tb hdd","pentium 14");
		p3spec = Arrays.asList("2gb ram","14 mp cam","64gb storage");
		p4spec = Arrays.asList("4gb ram","16`mp cam","128gb storage");
	...
	public void testProduct1() throws NotFoundException{
		...
		//verifying the order of methods invoked on mock instance
	...
	public void testProduct3() throws NotFoundException{
		//Enabling stubbing
		when(dao.getCatNProds()).thenReturn(catMap);
		when(dao.getProductSpecs("samsung galaxy7")).thenReturn(p3spec);
		
		Product expected = new Product(1003,"samsung galaxy7",12000);
		expected.setSpecList(p3spec);
		assertEquals(expected,ser.getProductInfo("mobile", 1003));
		
		InOrder order = Mockito.inOrder(dao);
		order.verify(dao).getCatNProds();
		order.verify(dao).getProductSpecs("samsung galaxy7");
	}
	...
	public void testProduct4() throws NotFoundException{
		//Enabling stubbing
		when(dao.getCatNProds()).thenReturn(catMap);
		when(dao.getProductSpecs("samsung s8")).thenReturn(p4spec);
		
		Product expected = new Product(1004,"samsung s8",24000);
		expected.setSpecList(p4spec);
		assertEquals(expected,ser.getProductInfo("mobile", 1004));
		
		
		//checks the order or calling
		InOrder order = Mockito.inOrder(dao);
		order.verify(dao).getCatNProds();
		order.verify(dao).getProductSpecs("samsung s8");
	}
	...
	public void testProduct6() throws NotFoundException{
		//Enabling stubbing
		when(dao.getCatNProds()).thenReturn(catMap);
		assertThrows(NotFoundException.class,()->ser.getProductInfo("mobile", 1002));
	}
	...
	public void testProduct7() throws NotFoundException{
		//Enabling stubbing
		when(dao.getCatNProds()).thenReturn(catMap);
		assertThrows(NotFoundException.class,()->ser.getProductInfo("aa", 1001));
	}
